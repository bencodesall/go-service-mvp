...
// TODO: Figure out why port list returns len => 0
...
func extractIPPort(t *testing.T, doc []map[string]interface{}, port string) (string, string) {
	...
	if !exists {
		t.Fatal("could not get network ports/tcp list settings; tcp.([]interface{}) does not exist")
	}
	// TODO: Figure out why port list returns len => 0
	//if len(list) != 1 {
	//	t.Fatal("could not get network ports/tcp list settings; len => 0")
	//}
